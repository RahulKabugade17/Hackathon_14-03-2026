...
const FINAL_REPORT_PATH = path.join(REPORTS_BASE_DIR, `cucumber-report-${timestamp}`);
...
if (!fs.existsSync(JSON_DIR)) {
    console.error(`\x1b[31mError: JSON directory does not exist: ${JSON_DIR}\x1b[0m`);
    process.exit(1);
}
...
if (!hasValidJson) {
    console.error('\x1b[31mError: No valid Cucumber JSON reports found in JSON directory!\x1b[0m');
...
try {
    ...
    reporter.generate({
        ...
        customData: {
            title: 'Project Info',
            data: [
                { label: 'Application', value: process.env.APP_NAME || 'Birla Opus App' },
                ...
                { label: 'Total Execution Time', value: process.env.TEST_DURATION || 'N/A' }
            ]
        },
        customData: {
            title: 'Project Info',
        ...
        disableLog: false,
        pageFooter: '<div style="margin: 20px;"><p>Birla Opus App Automation Report</p></div>'
    });

    console.log(`\x1b[32mReport generated successfully at: ${FINAL_REPORT_PATH}\x1b[0m`);
